# Запуск на rakurs.dev — чеклист

Сайт уже собирается под `https://rakurs.dev/` (константа `SITE` в
...
У регистратора: CNAME (или A/ALIAS, как скажет Gcore) корня `rakurs.dev` →
CNAME-цель CDN-ресурса Gcore. Дождаться выпуска HTTPS-сертификата на CDN.

## 3. Проверить и переотправить

1. https://rakurs.dev/ и https://rakurs.dev/en/ открываются, переключатель языка работает.
2. https://rakurs.dev/robots.txt и /llms.txt отдаются.
3. Rich Results Test: https://search.google.com/test/rich-results
   (Organization, ProfessionalService, FAQPage на `/`; Service на `/products/b1/`).
4. Google Search Console: property `rakurs.dev` → отправить
   `https://rakurs.dev/sitemap-index.xml`.
